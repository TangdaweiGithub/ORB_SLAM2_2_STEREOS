...
Viewer::Viewer(System* pSystem, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer, Tracking *pTracking, const string &strSettingPath, const int sensor):
    mpSystem(pSystem), mpFrameDrawer(pFrameDrawer),mpMapDrawer(pMapDrawer), mpTracker(pTracking),
    mbFinishRequested(false), mbFinished(true), mbStopped(true), mbStopRequested(false), mSensor(sensor)
{
...
void Viewer::Run()
{
    ...
    pangolin::CreateWindowAndBind("ORB-SLAM2_2_STEREOS: Map Viewer",1024,768);
    ...
    if (mSensor == System::TWOSTEREO)
        cv::namedWindow("ORB-SLAM2: Current Frame Back");
    ...
    while(1)
    {
        ...
        if (mSensor == System::TWOSTEREO)
            {
                cv::Mat imBack = mpFrameDrawer->DrawFrameBack();
                cv::imshow("ORB-SLAM2: Current Frame Back", imBack);
            }
