...
System::System(const string &strVocFile, const string &strSettingsFile, const eSensor sensor,
               const bool bUseViewer, const bool do_loadmap, const bool do_relocation):mSensor(sensor), mpViewer(static_cast<Viewer*>(NULL)), mbReset(false),mbActivateLocalizationMode(false),
        mbDeactivateLocalizationMode(false)
{
    ...
    mpFrameDrawer = new FrameDrawer(mpMap, mSensor);
    ...
    // for two stereos
    if (mSensor == TWOSTEREO)
    {
        mpLocalMapperBack = new LocalMappingBack(mpMap, mSensor == MONOCULAR);
        mptLocalMappingBack = new thread(&ORB_SLAM2::LocalMappingBack::Run, mpLocalMapperBack);
    }
    ...
    if(bUseViewer) 
    {
        ...
        mpViewer = new Viewer(this, mpFrameDrawer,mpMapDrawer,mpTracker,strSettingsFile, mSensor);
    ...
    mStrPathMap = "/home/dream/Documents/DreamVerse/ORB_SLAM2_2_STEREOS/MapPointandKeyFrame.bin";
    SystemSetting *mySystemSetting = new SystemSetting(mpVocabulary);  
    mySystemSetting->LoadSystemSetting(strPathSystemSetting);

    if(do_loadmap)
        mpMap->Load(mStrPathMap, mySystemSetting, mpKeyFrameDatabase);
    ...
    if (mSensor == System::TWOSTEREO)
        mpTracker->SetLocalMapperBack(mpLocalMapperBack);
    mpTracker->SetLoopClosing(mpLoopCloser);

    mpLocalMapper->SetTracker(mpTracker);
    mpLocalMapper->SetLoopCloser(mpLoopCloser);
    
        mpLocalMapperBack->SetLoopCloser(mpLoopCloser);
...
cv::Mat System::TrackTwoStereoWithIMU(const cv::Mat &imLeft, const cv::Mat &imRight, const cv::Mat &imBackLeft, const cv::Mat &imBackRight, const double &timestamp, const float current_yaw_angle_accums)
{
    if (mSensor != TWOSTEREO)
    {
        cerr << "ERROR: you called TrackTwoStereo but input sensor was not set to TWOSTEREO." << endl;
        exit(-1);
    }
    ...
    // Check mode change
    {
        ...
        {
            mpLocalMapper->RequestStop();
            mpLocalMapperBack->RequestStop();

            // Wait until Local Mapping has effectively stopped
            while (!mpLocalMapper->isStopped() || !mpLocalMapperBack->isStopped())
            {
                usleep(1000);
            }
    ...
    cv::Mat Tcw = mpTracker->GrabTwoImageStereo(imLeft, imRight, imBackLeft, imBackRight, timestamp);
