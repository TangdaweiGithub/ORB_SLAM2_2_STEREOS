...
void Map::Load ( const string &filename, SystemSetting* mySystemSetting, KeyFrameDatabase* mpKeyFrameDatabase )
{
    ...
    start = clock();
    ofstream outfile_mappoint("xyz_mappoint.txt", ios::trunc);
    ofstream outfile_mappoint_ue4("xyz_mappoint_ue4.txt", ios::trunc);
    for ( unsigned int i = 0; i < nMapPoints; i ++ )
    {
        MapPoint* mp = LoadMapPoint(f, outfile_mappoint, outfile_mappoint_ue4);
        AddMapPoint(mp);
    }
    cout << "Extract mappoint in xyz_mappoint.txt success !!!!!!" << endl;
    outfile_mappoint.close();
    outfile_mappoint_ue4.close();
    end = clock();
    cout << "load Map need time: " << (end-start)/CLOCKS_PER_SEC << endl;
    // Get all MapPoints
    ...
    ofstream outfile_keyframe("xyz_keyframe.txt", ios::trunc);
    ofstream outfile_keyframe_ue4("xyz_keyframe_ue4.txt", ios::trunc);
    ...
    for( unsigned int i = 0; i < nKeyFrames; i ++ )
    {
        ...
        kf = LoadKeyFrame(f, vmp, mySystemSetting, outfile_keyframe, outfile_keyframe_ue4);
        AddKeyFrame(kf);
        kf_by_order.push_back(kf);  
        mpKeyFrameDatabase->add(kf);
    }
    cout << "Extract keyframe pose in xyz_keyframe.txt success !!!!!!" << endl;
    outfile_keyframe.close();
    outfile_keyframe_ue4.close();
    end = clock();
    cout << "load keyframe need time: " << (end-start)/CLOCKS_PER_SEC << endl;
...
MapPoint* Map::LoadMapPoint( ifstream &f ,ofstream &outfile, ofstream &outfile_ue4)
{
    ...
    outfile << Position.at<float>(0) << " " << Position.at<float>(1) << " " << Position.at<float>(2) << endl;
    outfile_ue4 << Position.at<float>(2)*100 << " " << Position.at<float>(0)*100 << " " << -Position.at<float>(1)*100 << endl;
...
KeyFrame* Map::LoadKeyFrame(ifstream &f, std::vector<MapPoint*> &vmp, SystemSetting* mySystemSetting, ofstream &outfile, ofstream &outfile_ue4)
{
    ...
    outfile << Twc.at<float>(3,0) << " " << Twc.at<float>(3,1) << " " << Twc.at<float>(3,2) << endl;
    outfile_ue4 << Twc.at<float>(3,2)*100 << " " << Twc.at<float>(3,0)*100 << " " << -Twc.at<float>(3,1)*100 << endl;
